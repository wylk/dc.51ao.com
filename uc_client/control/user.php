...
class usercontrol extends base {
	...
	function onregister() {
		...
		$uid = $_ENV['user']->add_user($username, $openid, $head_url, 0, $questionid, $answer, $regip);
		return $uid;
	}

	function onregisters() {
		$this->init_input();
		$username = $this->input('username');
		...
		$uid = $_ENV['user']->add_user1($username, $password, $email, 0, $questionid, $answer, $regip);
		return $uid;
	}
	...
	function onlogin_openid() 
	{
		...
		$questionid = $this->input('questionid');
		$answer = $this->input('answer');
		$user = $_ENV['user']->get_user_by_openid($openid);
		if ($user) {
			return $user;
		}else{
			return false;
	...
	function _check_username1($username) {
		$username = addslashes(trim(stripslashes($username)));
